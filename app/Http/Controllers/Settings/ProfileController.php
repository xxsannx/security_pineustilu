...
class ProfileController extends Controller
{
    ...
    public function show(string $tab = 'profile') : \Illuminate\Contracts\View\View
    {
        ...
        switch ($tab) {
            ...
            case 'reschedule':
                $reschedules = Reschedule::query()
                    ->whereHas('oldBooking', fn ($q) => $q->where('user_id', $user->id)->orWhere('guest_email', $user->email))
                    ->with([
                        'oldBooking.bookingDetails.unit.area',
                        'oldBooking.bookingOutbounds.outbound',
                        'oldBooking.bookingOutbounds.outboundVariant',
                        'newBooking.bookingDetails.unit.area',
                        'newBooking.bookingOutbounds.outbound',
                        'newBooking.bookingOutbounds.outboundVariant',
                    ])
                    ->latest('created_at')
                    ->get();

                $data['reschedules'] = $reschedules->map(fn (Reschedule $reschedule) => $this->toRescheduleCard($reschedule));
                break;
    ...
    private function toRescheduleCard(Reschedule $reschedule) : object
    {
        ...
        return (object) array_merge((array) ($preferred ?? []), [
